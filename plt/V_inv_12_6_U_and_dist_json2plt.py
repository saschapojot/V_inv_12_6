...
jsonFolderRoot="../dataAll/row"+str(rowNum)+"/jsonOutAll/"

inCsvFile="../V2Params.csv"

# dfParams=pd.read_csv(inCsvFile,dtype={"a1":float,"a2":float,"c1":float,"c2":float})
# rowVals=dfParams.iloc[rowNum,:]
# a1=rowVals["a1"]
# a2=rowVals["a2"]
# c1=rowVals["c1"]
# c2=rowVals["c2"]
...
def pltU_dist(oneTFile):
    ...
    U_distPath=oneTFile+"/U_dist/U_distData.json"
    with open(U_distPath,"r") as fptr:
        data=json.load(fptr)

    UVec=np.array(data["U"])
    LVec=np.array(data["L"])
    y0Vec=np.array(data["y0"])
    z0Vec=np.array(data["z0"])
    y1Vec=np.array(data["y1"])
